...
public class CustomerManager {
    ...
    private void initializeDatabase() {
        try {
            ...
            executor.executeQScript(qScript, kdbHost, kdbPort);
            ...
            if (result instanceof c.Flip) {
                ...
                for (int i = 0; i < ids.length; i++) {
                    ...
                    if (region.startsWith("S")) {
                        logger.info(String.format("ID: %d, Name: %s, Surname: %s, Country: %s, Age: %d, Local Time: %s, Tel Num: %s, Region: %s",
                                id, name, surname, country, age, localTime, telNum, region));
                    }
                }
            }
        } catch (Exception e) {
